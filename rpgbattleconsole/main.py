    ...
    for player in players:
        ...
        # if player select magic > select spell
            ...
            # if player selected black magic > attack
                ...
                print("\n You attacked", enemies[enemy].name.replace(" ", ""), "for", magic_dmg,
                      "points of damage with ", spell.name, " And cost ", spell.cost, " :")
                enemies[enemy].get_enemy_stats()
        ...
        # if player select items > select item
            ...
            # if player select item with type attack > attack
                ...
                print("You attacked", enemies[enemy].name.replace(" ", ""), "for", item.prop, "points of damage with ",
                      item.name)
    ...
    for enemy in enemies:
        ...
        # if enemy choice is 0 {magic} then select spell
            ...
            if spell.type == "white":
                ...
                print("\n", enemy.name.replace(" ", ""), "Enemy healed ! HP +", magic_dmg, "points. with ", spell.name,
                      " And cost ", spell.cost)
                enemy.get_stats()
